...
class ls_shop_beModule_stockManagement extends \BackendModule {
	...
	protected function compile() {
		...
		if (\Input::post('FORM_SUBMIT') == 'changeStock') {
			...
			if (!preg_match('/^(\+|-)?[0-9]*\.?[0-9]*$/', $quantity)) {
				...
				if ($blnStockChangeCarriedOut) {
                    $session_lsShopCart['stockManagement']['successMsg'][\Input::post('productVariantID')] =  $GLOBALS['TL_LANG']['be_stockManagement']['text018'];
		...
		if (is_array($session_lsShopCart['beModule_productSearch']['values'])) {
