...
class dynamicAttachment_invoice_01 extends Controller {
    ...
    // The filename and -path to be used is defined here. The placeholder will be replaced with the messageCounterNr later.
    private $filepath = 'files/merconisfiles/dynamicAttachmentFiles/generatedFiles/';
    private $filename = 'invoice_%s.pdf';
    ...
    public function __construct($arrOrder = array(), $messageCounterNr = null) {
        ...
        if (!file_exists($projectDir.'/'.$this->filepath)) {
            mkdir($projectDir.'/'.$this->filepath, 0775, true);
        }

        if (file_exists($projectDir.'/'.$this->filename)) {
    ...
    private function addPage($skipTableHeadline = false) {
         ...
         * Background image
         */
        $bgImage = System::getContainer()->getParameter('kernel.project_dir').'/files/merconisfiles/theme/dynamicAttachmentFiles/images/invoice_background_01.gif';
        ...
        $this->pdf->Cell(0, 0, Date::parse($GLOBALS['TL_CONFIG']['dateFormat'], time()));
