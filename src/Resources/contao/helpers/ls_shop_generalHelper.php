...
class ls_shop_generalHelper
{
    ...
    public static function LaFP()
    {
        if (!isset($GLOBALS['merconis_globals']['LaFP']['y2'])) {
            ...
            //if ($GLOBALS['TL_CONFIG']['gracePeriodDaysLeft'] != (999999)) { \Contao\Config::getInstance()->update("\$GLOBALS['TL_CONFIG']['gracePeriodDaysLeft']", (999999)); \Contao\Config::getInstance()->save();}
            ...
            if ($str_sn && substr(md5($str_sn), 0, 5) == $str_hs) {
                ...
                //\Contao\Config::getInstance()->update("\$GLOBALS['TL_CONFIG']['gracePeriodDaysLeft']", (ceil((hexdec($str_dl) - (time() - $ut_LaFP) / (86400))))); \Contao\Config::getInstance()->save();
