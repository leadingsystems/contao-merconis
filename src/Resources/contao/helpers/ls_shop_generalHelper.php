...
class ls_shop_generalHelper
{
    ...
    /*
     * FRAGE: Ist es wichtig, dass $obj_productOrVariant hier als Referenz verwendet wird?
     * Vermutung: Ja, weil das Objekt als Argument an den Konstruktor der Customizer-Klasse übergeben wird
     * und dann dort zur direkten Manipulation bereit steht.
     */
    public static function getCustomizerObject(&$obj_productOrVariant) {
        ...
        if (
            !$obj_productOrVariant->_hasCustomizerLogicFile
            && (
                $obj_productOrVariant->_objectType !== 'variant'
                || !$obj_productOrVariant->_objParentProduct->_hasCustomizerLogicFile
            )
        ) {
            /*
        ...
        if ($obj_productOrVariant->_hasCustomizerLogicFile) {
            $obj_productOrVariantToGetCustomizerFrom = $obj_productOrVariant;
            $obj_productOrVariantToManipulateByCustomizer = &$obj_productOrVariant;
        } else {
            $obj_productOrVariantToGetCustomizerFrom = $obj_productOrVariant->_objParentProduct;
            $obj_productOrVariantToManipulateByCustomizer = &$obj_productOrVariant;
        }


        $str_customizerObjectKey = $obj_productOrVariantToGetCustomizerFrom->_customizerLogicFile . '_' . $obj_productOrVariantToGetCustomizerFrom->ls_productVariantID . ($obj_productOrVariantToGetCustomizerFrom->_configuratorHash ? '|' . $obj_productOrVariantToGetCustomizerFrom->_configuratorHash : '');

        if (!isset($GLOBALS['merconis_globals']['customizerObjects'])) {
            $GLOBALS['merconis_globals']['customizerObjects'] = [];
        }

        require_once(TL_ROOT ."/". $obj_productOrVariantToGetCustomizerFrom->_customizerLogicFile);
        $str_customLogicClassName = '\Merconis\Core\\'.preg_replace('/(^.*\/)([^\/\.]*)(\.php$)/', '\\2', $obj_productOrVariantToGetCustomizerFrom->_customizerLogicFile);
        ...
        $GLOBALS['merconis_globals']['customizerObjects'][$str_customizerObjectKey] = new $str_customLogicClassName($obj_productOrVariantToManipulateByCustomizer, $obj_productOrVariantToGetCustomizerFrom->_configuratorHash);
...
//				throw new \Exception('Required form not found (formID: '.$formID.')');
    ...
    public static function getBackendLscssStyles() {
        ...
        return $obj_lscss4cController->getLscss(
            ls_getFilePathFromVariableSources($GLOBALS['TL_CONFIG']['ls_shop_lscssFileToLoad']) ?: '/vendor/leadingsystems/contao-merconis/src/Resources/public/lscss/lscss-backend-project.scss',
