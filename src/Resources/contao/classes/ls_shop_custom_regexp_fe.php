			...
			switch($strRegexp) {
				case 'merconisCheckVATID':
					...
					if (isset($GLOBALS['TL_CONFIG']['ls_shop_useVATIDValidation']) && $GLOBALS['TL_CONFIG']['ls_shop_useVATIDValidation']) {
						 ...
						 */
						if (
								!isset($arrLsShop['checkedVATID'][$varValue])
							||	!is_array($arrLsShop['checkedVATID'][$varValue])
						) {
							...
							if ($result === null || !isset($result->valid) || ($result->valid !== true && $result->valid !== false && $result->valid !== 1 && $result->valid !== 0)) {
                                $arrLsShop['checkedVATID'][$varValue] = array(
									'valid' => null
								);
							} else {
                                $arrLsShop['checkedVATID'][$varValue] = array(
						 ...
						 */
						if ($arrLsShop['checkedVATID'][$varValue]['valid'] === null || $arrLsShop['checkedVATID'][$varValue]['valid']) {
							/*
							 * If $arrLsShop['checkedVATID'][$varValue]['valid'] is null (no result could be retrieved for whatever reason, e. g. soap service down)
