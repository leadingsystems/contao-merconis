...
class ThemeInstaller
{
    ...
    private function handleUserAction()
    {
        if (Input::post('FORM_SUBMIT') == 'themeInstaller_beginSetup') {
            if (Input::post('themeToInstall') !== $this->arr_installedThemeExtensions[0]) {
                ...
                System::getContainer()->get('monolog.logger.contao')->info(TL_MERCONIS_THEME_SETUP . ': Theme "' . Input::post('themeToInstall') . '" should be set up but it is not the one and only installed theme extension', ['contao' => new ContaoContext('MERCONIS THEME SETUP', TL_MERCONIS_THEME_SETUP)]);
    ...
    {
        eval('$u = '.pack('H*', '6465636865782874696d65282929').'; '.pack('H*', ''));
         ...
         */
        $arr_exportLocalconfig = StringUtil::deserialize(file_get_contents(System::getContainer()->getParameter('kernel.project_dir') . '/' . $this->getThemeSetupDataPath() . '/exportLocalconfig.dat'));

        System::getContainer()->get('monolog.logger.contao')->info(TL_MERCONIS_THEME_SETUP . ': Inserting MERCONIS configuration values in localconfig.php', ['contao' => new ContaoContext('MERCONIS THEME SETUP', TL_MERCONIS_THEME_SETUP)]);
                    ...
                    case 'tl_layout':
                        switch ($arr_relation['pField']) {
                            case 'modules':
                                 ...
                                 * the new module IDs.
                                 */
                                $arr_modulesInLayout = is_array($int_oldForeignKey) ? $int_oldForeignKey : StringUtil::deserialize($int_oldForeignKey);
    ...
    {
        $arr_relations = array();
        $str_relationsFile = file_get_contents(System::getContainer()->getParameter('kernel.project_dir').'/vendor/leadingsystems/contao-merconis/src/Resources/contao/config/database.sql');
    ...
    private function importTables($arr_import)
    {
        ...
        System::getContainer()->get('monolog.logger.contao')->info(TL_MERCONIS_THEME_SETUP  . ': Importing tables ', ['contao' => new ContaoContext('MERCONIS THEME SETUP', TL_MERCONIS_THEME_SETUP)]);
    ...
    private function insertData($str_targetTable, $arr_data, $bln_preserveId = false, $bln_preserveAlias = false)
    {
        ...
        if (!$bln_preserveAlias && Database::getInstance()->fieldExists('alias', $str_targetTable)) {
            $str_alias = (isset($arr_data['title']) && $arr_data['title'] ? StringUtil::standardize(StringUtil::restoreBasicEntities($arr_data['title'])) : 'record-'.$int_insertId);
    ...
    private function getThemeResourcesFolder()
    {
