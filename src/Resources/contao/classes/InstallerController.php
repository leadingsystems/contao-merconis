...
//		\System::log('MERCONIS INSTALLER: Check if installation is possible', 'MERCONIS INSTALLER', TL_MERCONIS_ERROR);
	...
	protected function checkIfThemeCanBeInstalled() {
		...
		if (!file_exists(TL_ROOT.'/'.$_SESSION['lsShop']['installer_selectedTheme']['srcPathExportTablesDat'])) {
			...
			$arrExportTables = \Contao\StringUtil::deserialize(file_get_contents(TL_ROOT.'/'.$_SESSION['lsShop']['installer_selectedTheme']['srcPathExportTablesDat']));
		...
		if (!file_exists(TL_ROOT.'/'.$_SESSION['lsShop']['installer_selectedTheme']['srcPathExportLocalconfigDat'])) {
			...
			$arrExportLocalconfig = \Contao\StringUtil::deserialize(file_get_contents(TL_ROOT.'/'.$_SESSION['lsShop']['installer_selectedTheme']['srcPathExportLocalconfigDat']));
	...
	protected function getNewForeignKey($relation, $oldForeignKey) {
		switch ($relation['relationType']) {
			...
			case 'array': // Der ForeignKey ist ein (serialisiertes) Array
				...
				$arrOldForeignKeys =  is_array($oldForeignKey) ? $oldForeignKey : \Contao\StringUtil::deserialize($oldForeignKey);
					...
					case 'tl_layout':
						switch ($relation['pField']) {
							case 'modules':
								 ...
								 */
								$arrModulesInLayout = is_array($oldForeignKey) ? $oldForeignKey : \Contao\StringUtil::deserialize($oldForeignKey);
	...
	public function checkForUpdateSituation() {
		...
		if ($currentProgramFilesVersion == $installedVersion) {
			...
			if (isset($GLOBALS['TL_CONFIG']['ls_shop_updateStatus'])) {
				$GLOBALS['merconis_globals']['update']['arrUpdateStatus'] = \Contao\StringUtil::deserialize($GLOBALS['TL_CONFIG']['ls_shop_updateStatus'], true);
			}
