...
class ls_shop_checkout {
    ...
    // will be filled after checkout is finished, use the getters
    private $orderId = null;
    private $str_paymentAfterCheckoutUrlWithOih = null;
    private $str_afterCheckoutUrlWithOih = null;
    private $bln_usePaymentAfterCheckoutPage = null;

    public function __construct() {

    }

    private function assertCheckoutDone() {
        if (!$this->isCheckoutDone) {
            throw new \Exception('Checkout has not been completed yet, only use getter if in ls_shop_checkout if checkout is finished');
        }
    }
    ...
    function getPaymentAfterCheckoutUrlWithOih(){
        $this->assertCheckoutDone();
        return $this->str_paymentAfterCheckoutUrlWithOih;
    }

    function getAfterCheckoutUrlWithOih(){
        $this->assertCheckoutDone();
        return $this->str_afterCheckoutUrlWithOih;
    }

    function hasUsePaymentAfterCheckoutPage(){
        $this->assertCheckoutDone();
        return $this->bln_usePaymentAfterCheckoutPage;
    }
    ...
    function completeCheckout($extraDataToSaveInCreatedOrder = [])
    {
         ...
         */
        $orderIdInDb = $this->saveOrderInDB($order, $extraDataToSaveInCreatedOrder);
        ...
        $this->bln_usePaymentAfterCheckoutPage = $bln_usePaymentAfterCheckoutPage;
        $this->str_paymentAfterCheckoutUrlWithOih = $str_paymentAfterCheckoutUrlWithOih;
        $this->str_afterCheckoutUrlWithOih = $afterCheckoutUrlWithOih;
        $this->orderId = $orderIdInDb;
    ...
    public function generateOrderNr() {
        ...
        $config->update("\$GLOBALS['TL_CONFIG']['ls_shop_orderNrCounter']", $nextCounter);
        $config->persist("\$GLOBALS['TL_CONFIG']['ls_shop_orderNrCounter']", $nextCounter);
        if ($useTableLock) {
            Database::getInstance()->unlockTables();
        }
    ...
    public function saveOrderInDB($order, $extraDataToSaveInCreatedOrder= []) {
        ...
        // Standardwerte in Parameterliste
        $params = [
        ...
        foreach ($extraDataToSaveInCreatedOrder as $column => $value) {
            $sql .= ", `$column` = ?";
            $params[] = $value;
        }

        $objInsertedOrder = Database::getInstance()
            ->prepare($sql)
            ->execute(...$params);
