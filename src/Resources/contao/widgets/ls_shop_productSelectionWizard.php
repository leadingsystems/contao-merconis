...
class ls_shop_productSelectionWizard extends Widget {
	...
	<li>'.$objWidget->generate();
			...
			foreach ($arrButtons as $button) {
				$return .= '<a class="button-'.$button.'" href="'.$this->addToUrl('&amp;'.$strCommand.'='.$button.'&amp;cid='.$i.'&amp;id='.$this->currentRecord).'" title="'.StringUtil::specialchars($GLOBALS['TL_LANG']['MSC']['lw_'.$button]).'" onclick="ls_shop_backend.listWizard(this,\''.$button.'\',\'ctrl_'.$this->strId.'\'); return false">'.Image::getHtml($button.'.svg', $GLOBALS['TL_LANG']['MSC']['lw_'.$button], 'class="tl_listwizard_img"').'</a> ';
			}
			$return .= '</div>';
